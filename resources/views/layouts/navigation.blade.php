...
<!-- Tabela Inicial de filmes -->
@can('view_table')
    <x-nav.link route='table'  icon="fas fa-film">{{ ('Lista de Filmes') }}</x-nav.link>
@endcan

@can('view_adicionar')
    <x-nav.link route='adicionar' icon="fas fa-circle-plus" >{{ ('Adicionar Filmes') }}</x-nav.link>
@endcan
